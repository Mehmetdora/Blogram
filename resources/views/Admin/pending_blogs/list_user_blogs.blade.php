            ...
            <div class="row mb-4" >
                <div class="col-sm-11" style="display: flex; max-height:50px">
                    <input id="search" class="form-control me-2 " type="search"
                        placeholder="Search by id, title, description..." data-user-id="{{$blogs_owner->id}}" onfocus="this.value=''" aria-label="Search">
                </div>
            </div>
                    ...
                    <div class="col-lg-3 mb-3" style="background-color:whitesmoke; margin:5px; border-radius:7px; ">
                        <div class="post-entry-alt">
                            ...
                            <div class="excerpt">
                                ...
                                <h2><a href="{{ route('detail-pending_blog', $blog->id) }}"> {{ $blog->title }} </a></h2>
                                <div class="post-meta align-items-center text-left clearfix">
                                    <figure class="author-figure mb-0 me-3 float-start">
                                        @if ($blog->user->photo)
                                            <img src="{{ asset('uploads/' . $blog->user->photo) }}" class="img-fluid"
                                                alt="Author Image">
                                        @else
                                ...
                                <p> {{ $blog->summery }} </p>
                                <p><a href="{{ route('detail-pending_blog', $blog->id) }}" class="read-more">More Info</a></p>
                            </div>
                        </div>
                    </div>
                    @endforeach
